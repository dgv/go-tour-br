		...
		if ($h2.length > 0) {
			...
			if (i > 0) {
				$nav.append($("<a>◀</a>").click(function() {
					show(i-1);
					return false;
				}).attr("href", "#"+(i)).attr("title", "Anterior"));
			} else {
			...
			if (i+1 < slides.length) {
				$nav.append($("<a>▶</a>").click(function() {
					show(i+1);
					return false;
				}).attr("href", "#"+(i+2)).attr("title", "Próximo"));
			} else {
				$nav.append($("<span>▶</span>"));
			}
			$nav.insertBefore($h2);
...
function run() {
	seq++;
	var cur = seq;
	$output.html('<div class="loading">Esperando o servidor remoto ...</div>');
	$.ajax("/compile", {
		...
		error: function() {
			$output.empty();
			$('<pre class="error" />').text("Erro de comunicação com o servidor remoto.").appendTo($output);
		}
	});
}
